...
class PetsCubit extends Cubit<PetsState> {
  ...
  final LocalStorageRepository localStorageRepository = LocalStorageRepositoryImpl( dataSource: IsarLocalDbDataSource() );
  ...
  Future<bool> editPet( Pet petUpdated ) async {
    ...
    emit(
      state.copyWith(
        pets: state.pets.map(
          ( pet ) => ( pet.id == petUpdated.id ) ? petUpdated : pet
        ).toList()
      )
    );
